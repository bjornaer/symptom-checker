...
// The AilmentFunc type is an adapter to allow the use of ordinary
// function as Ailment mutator.
type AilmentFunc func(context.Context, *ent.AilmentMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f AilmentFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.AilmentMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.AilmentMutation", m)
	}
	return f(ctx, mv)
}
